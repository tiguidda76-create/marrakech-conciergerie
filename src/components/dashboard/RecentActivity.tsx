...
export function RecentActivity({ bookings, tasks }: RecentActivityProps) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="p-5 sm:p-6 rounded-card bg-surface border border-surface-border shadow-lg flex flex-col justify-between">
        <div>
          ...
          <div className="space-y-3">
            {bookings.length === 0 ? (
              <div className="p-8 text-center rounded-lg bg-surface-elevated/40 border border-surface-border text-xs text-muted-foreground">
                <p>Aucune réservation enregistrée.</p>
                <Link href="/reservations" className="text-primary hover:underline font-bold mt-2 inline-block">
                  ➕ Ajouter une réservation réelle
                </Link>
              </div>
            ) : (
      ...
      {/* Operations & Urgent Tasks */}
      <div className="p-5 sm:p-6 rounded-card bg-surface border border-surface-border shadow-lg flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-serif text-base font-bold text-foreground">Tâches & Opérations Terrains</h2>
          ...
          <div className="space-y-3">
            {tasks.length === 0 ? (
              <div className="p-8 text-center rounded-lg bg-surface-elevated/40 border border-surface-border text-xs text-muted-foreground">
                <p>Aucune tâche opérationnelle en attente.</p>
                <Link href="/taches" className="text-primary hover:underline font-bold mt-2 inline-block">
                  ➕ Créer une tâche (Ménage, Check-in)
                </Link>
              </div>
            ) : (
